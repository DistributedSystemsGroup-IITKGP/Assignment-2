...
class TestServer(unittest.TestCase):
    ...
    def test_enqueue(self):
        ...
        response = self.client.post('/producer/produce', json={"topic_name":"test_topic", "producer_id":0, "log_message":"test log message"})
        data = response.get_json()
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data['status'], 'success')
    ...
    def test_dequeue(self):
        ...
        self.client.post('/producer/produce', json={"topic_name":"test_topic", "producer_id":0, "log_message":"test log message"})

        response = self.client.get('/consumer/consume', json={'topic_name':'test_topic', "consumer_id":0})
        data = response.get_json()
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data['status'], 'success')
    ...
    def test_size(self):
        ...
        self.client.post('/producer/register', json={'topic_name': 'test_topic'})
        self.client.post('/consumer/register', json={'topic_name': 'test_topic'})
        self.client.post('/producer/produce', json={"topic_name":"test_topic", "producer_id":0, "log_message":"test log message 1"})
        self.client.post('/producer/produce', json={"topic_name":"test_topic", "producer_id":0, "log_message":"test log message 2"})
        print(self.client.get('/consumer/consume', json={'topic_name':'test_topic', "consumer_id":0}).get_json())
        print(self.client.get('/consumer/consume', json={'topic_name':'test_topic', "consumer_id":0}).get_json())

        response = self.client.get('/size', json={"topic_name":"test_topic", "consumer_id":0})
        data = response.get_json()
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data['status'], 'success')
