...
curl -X POST -H "Content-Type: application/json" -d '{"topic_name":"example_topic", "producer_id":0, "log_message":"example log message 1"}' http://localhost:5000/producer/produce

curl -X POST -H "Content-Type: application/json" -d '{"topic_name":"example_topic", "producer_id":0, "log_message":"example log message 2"}' http://localhost:5000/producer/produce
